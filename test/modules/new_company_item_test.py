...
class NewCompanyTest(unittest.TestCase):
    ...
    def test_save_should_write_db(self):
        item = NewCompanyItem(self.dbfile)
        item.save(1, 'spoons', 1000, 'metal')

        with open(self.dbfile, 'r') as db:
            content = db.readlines()
            self.assertEqual(content, ['1, spoons, 1000, metal\n'])
    ...
    def test_delete_should_delete_the_correct_item_from_db(self):
        ...
        with open(self.dbfile, 'r') as db:
            content = db.readlines()
            self.assertEqual(content, ['2, forks, 1000, metal\n'])
    
    def test_search_shoud_return_the_item(self):
        item = NewCompanyItem(self.dbfile)
        item.save(1, 'spoons', 1000, 'metal')

        self.assertEqual(['1, spoons, 1000, metal\n'], item.search('spoons'))
